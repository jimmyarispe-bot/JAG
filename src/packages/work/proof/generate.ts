...
import { buildWorkProofOrganizationBlueprint } from "@/packages/work/proof/organization";
import { createWorkModelCompilerPorts } from "@/packages/work/proof/ports";

export function generateWorkProofRuntime(): GenerateRuntimeSpecificationResult {
  return generateRuntimeSpecification({
    industry: EducationIndustryBlueprint,
    organization: buildWorkProofOrganizationBlueprint(),
  });
}

export function compileWorkProofRuntime(
  ...
  const generation = generateWorkProofRuntime();
  if (!generation.ok || !generation.specification) {
    return {
      ...
      error: generation.error ?? {
        ...
        message: "Work proof generation failed",
      },
    };
  }

  const compiled = compileApplicationModel(generation.specification, {
    ports: createWorkModelCompilerPorts(),
