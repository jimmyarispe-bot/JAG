/**
 * Scheduling pack helpers — third-party style entity builders (definitions only).
 */

import type { EntityModel } from "@/jag/modeling";
import { SCHEDULING_APPLICATION_ID } from "@/packages/scheduling/package";
...
export function schedulingEntity(input: {
  ...
  const prefix = input.permissionPrefix ?? `scheduling.${input.entityType}`;
