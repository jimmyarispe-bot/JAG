/**
 * Reporting pack helpers — third-party style entity builders (definitions only).
 */

import type { EntityModel } from "@/jag/modeling";
import { REPORTING_APPLICATION_ID } from "@/packages/reporting/package";
...
export function reportingEntity(input: {
  entityType: string;
  label: string;
  ...
  const prefix = input.permissionPrefix ?? `reporting.${input.entityType}`;
