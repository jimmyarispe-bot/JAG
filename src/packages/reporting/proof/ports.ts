...
export function resetReportingProofPortsForTests(): void {
  permissionPacks.clear();
}

export function listReportingProofPermissionPacks(): PermissionModel[] {
  return [...permissionPacks.values()].sort((a, b) => a.id.localeCompare(b.id));
}

export function createReportingModelCompilerPorts(): ApplicationModelCompilerPorts {
  return {
