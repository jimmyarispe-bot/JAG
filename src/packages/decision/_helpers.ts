/**
 * Decision pack helpers — third-party style entity builders (definitions only).
 */

import type { EntityModel } from "@/jag/modeling";
import { DECISION_APPLICATION_ID } from "@/packages/decision/package";
...
export function decisionEntity(input: {
  entityType: string;
  label: string;
  ...
  const prefix = input.permissionPrefix ?? `decision.${input.entityType}`;
