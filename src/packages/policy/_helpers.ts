/**
 * Policy pack helpers — third-party style entity builders (definitions only).
 */

import type { EntityModel } from "@/jag/modeling";
import { POLICY_APPLICATION_ID } from "@/packages/policy/package";
...
export function policyEntity(input: {
  entityType: string;
  label: string;
  ...
  const prefix = input.permissionPrefix ?? `policy.${input.entityType}`;
