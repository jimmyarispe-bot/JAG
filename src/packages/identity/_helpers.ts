/**
 * Identity pack helpers — third-party style entity builders (definitions only).
 */

import type { EntityModel } from "@/jag/modeling";
import { IDENTITY_APPLICATION_ID } from "@/packages/identity/package";
...
export function identityEntity(input: {
  entityType: string;
  label: string;
  ...
  const prefix = input.permissionPrefix ?? `identity.${input.entityType}`;
