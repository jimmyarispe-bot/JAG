/**
 * Analytics pack helpers — third-party style entity builders (definitions only).
 */

import type { EntityModel } from "@/jag/modeling";
import { ANALYTICS_APPLICATION_ID } from "@/packages/analytics/package";
...
export function analyticsEntity(input: {
  entityType: string;
  label: string;
  ...
  const prefix = input.permissionPrefix ?? `analytics.${input.entityType}`;
