...
export function statusFromScore(score: number): CulturalHealthStatus { return sharedStatusFromScore(score); }
export function priorityFromScore(score: number): CulturalPriorityBand { return priorityFromScoreLowUrgent(score); }
export function levelFromValue(value: number): CulturalConfidenceLevel { return sharedLevelFromValue(value); }
export function outlookFromScore(score: number, volatility = 0): CulturalOutlook {
  return outlookFromScoreConfigured(score, volatility, {
    ...
    high: { min: OUTLOOK_THRESHOLDS_STANDARD.high, label: "cohesive" },
    mid: { min: OUTLOOK_THRESHOLDS_STANDARD.mid, label: "stable" },
    low: { min: OUTLOOK_THRESHOLDS_STANDARD.low, label: "fragmented" },
    fallback: "uncertain",
  });
}
export function buildConfidence(
  factors: Array<{ key: string; label: string; contribution: number }>
): CulturalConfidenceScore {
  return buildConfidenceAverage(factors) as CulturalConfidenceScore;
}
...
export const emptyCulturalScope = (): GraphScope => emptyGraphScope();
