import { buildConfidence } from "@/lib/platform/intelligence/behavioral/models";
import type { BehavioralProjectionResult, BehavioralQueryRequest, BehavioralQueryResult, BehavioralResult } from "@/lib/platform/intelligence/behavioral/types";

export class BehavioralProjection {
  project(input: Omit<BehavioralProjectionResult, "forecast">): BehavioralProjectionResult {
    return { ...input, forecast: Math.min(100, input.healthScore + (input.outlook === "adaptive" ? 6 : input.outlook === "resistant" ? -4 : 2)) };
  }
}

export class BehavioralQueries {
  ask(result: BehavioralResult, request: BehavioralQueryRequest): BehavioralQueryResult {
    ...
    else if (focus === "recommendations") { answer = `${result.recommendations.length} behavioral recommendations.`; }
