...
export function outlookFromScore(score: number, volatility = 0): BehavioralOutlook {
  ...
  if (score >= 78) return "adaptive"; if (score >= 62) return "stable"; if (score >= 45) return "resistant"; return "uncertain";
}
export function buildConfidence(factors: Array<{ key: string; label: string; contribution: number }>): BehavioralConfidenceScore {
...
export const emptyBehavioralScope = (): GraphScope => ({ organizationId: null, schoolId: null });
