import { buildConfidence } from "@/lib/platform/intelligence/economic/models";
import type { EconomicProjectionResult, EconomicQueryRequest, EconomicQueryResult, EconomicResult } from "@/lib/platform/intelligence/economic/types";

export class EconomicProjection {
  project(input: Omit<EconomicProjectionResult, "forecast">): EconomicProjectionResult {
    return { ...input, forecast: Math.min(100, input.healthScore + (input.outlook === "expansionary" ? 6 : input.outlook === "contractionary" ? -4 : 2)) };
  }
}

export class EconomicQueries {
  ask(result: EconomicResult, request: EconomicQueryRequest): EconomicQueryResult {
    ...
    else if (focus === "recommendations") { answer = `${result.recommendations.length} economic recommendations.`; }
