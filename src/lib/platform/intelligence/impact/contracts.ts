/**
 * Impact Intelligence — contracts / interfaces only.
 *
...
export interface ImpactRepository {
  save(result: T.ImpactResult): T.ImpactResult;
  get(requestId: string): T.ImpactResult | null;
  list(scope?: Partial<T.GraphScope>): T.ImpactResult[];
  remove(requestId: string): boolean;
  saveHistory(record: T.ImpactHistoryRecord): T.ImpactHistoryRecord;
  listHistory(scope?: Partial<T.GraphScope>): T.ImpactHistoryRecord[];
  clear(): void;
}
export interface ImpactRegistry {
  register(domain: string, capability: string): void;
  list(): T.ImpactPublisher[];
  isRegistered(domain: string): boolean;
  clear(): void;
}
export interface ImpactIntelligenceService {
  build(request: T.ImpactRequest): T.ImpactResult;
  query(result: T.ImpactResult, request: T.ImpactQueryRequest): T.ImpactQueryResult;
