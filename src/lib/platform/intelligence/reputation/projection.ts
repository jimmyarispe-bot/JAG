import { buildConfidence } from "@/lib/platform/intelligence/reputation/models";
import type { ReputationProjectionResult, ReputationQueryRequest, ReputationQueryResult, ReputationResult } from "@/lib/platform/intelligence/reputation/types";

export class ReputationProjection {
  project(input: Omit<ReputationProjectionResult, "forecast">): ReputationProjectionResult {
    return { ...input, forecast: Math.min(100, input.healthScore + (input.outlook === "ascending" ? 6 : input.outlook === "fragile" ? -4 : 2)) };
  }
}

export class ReputationQueries {
  ask(result: ReputationResult, request: ReputationQueryRequest): ReputationQueryResult {
    ...
    else if (focus === "recommendations") { answer = `${result.recommendations.length} reputation recommendations.`; }
