import { buildConfidence } from "@/lib/platform/intelligence/stakeholder/models";
import type { StakeholderProjectionResult, StakeholderQueryRequest, StakeholderQueryResult, StakeholderResult } from "@/lib/platform/intelligence/stakeholder/types";

export class StakeholderProjection {
  project(input: Omit<StakeholderProjectionResult, "forecast">): StakeholderProjectionResult {
    return { ...input, forecast: Math.min(100, input.healthScore + (input.outlook === "aligned" ? 6 : input.outlook === "strained" ? -4 : 2)) };
  }
}

export class StakeholderQueries {
  ask(result: StakeholderResult, request: StakeholderQueryRequest): StakeholderQueryResult {
    ...
    else if (focus === "recommendations") { answer = `${result.recommendations.length} stakeholder recommendations.`; }
