...
/** Document 98 §7 — PA atomic skills (draft until content review). */
  ...
  for (const record of records) {
    for (const ref of record.futureAtomicSkillRefs) {
      skills.push({
        ...
        title: ref.title,
        description: `Doc 98 draft atomic skill for ${record.title}`,
        version: "1.0.0",
        status: "draft",
        prerequisites: [],
        relatedSkills: [],
        ...
        masteryCriteria: "≥ 4/5 trials per Doc 51 probe threshold",
...
/** All SL atomic skills — Doc 98 PA drafts + generated library entries. */
