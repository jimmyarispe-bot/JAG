...
import { getSyncRegistry } from "@/lib/platform/integrations/microsoft-365/sync/registry-store";
import { listRecentSyncRuns } from "@/lib/platform/integrations/microsoft-365/sync/run-repository";
import type { MicrosoftSyncProgressStatus } from "@/lib/platform/integrations/microsoft-365/sync/types";
...
export async function getMicrosoft365SyncProgress(
  supabase: AuthClient,
  organizationId: string
): Promise<MicrosoftSyncProgressStatus> {
  const connection = await loadConnection(supabase, organizationId);
  const row = connection;
  const snap = microsoft365Store.get(organizationId);
  const pointer = connection
    ? memoryMicrosoftSyncRegistry.connectionPointers.get(connection.id)
    : null;
