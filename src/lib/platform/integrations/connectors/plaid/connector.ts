/**
 * Production Plaid connector — implements shared Connector contract.
 * Plaid remains banking connectivity; FIs remain SoR; JAG syncs/normalizes/caches.
 */
...
import type { PlaidEnvironment, PlaidRawEntity } from "./entities";
import {
  allPlaidObjectTypes,
  createDemoPlaidClient,
  type PlaidClient,
} from "./client";
import { plaidMetadata } from "./metadata";
import { normalizePlaidRecords, plaidCanonicalType, toSyncRecords } from "./normalize";
import { plaidStore } from "./store";

export type CreatePlaidConnectorOptions = {
  client?: PlaidClient;
};

export function createPlaidConnector(
  deps: PlaceholderConnectorDeps,
  options: CreatePlaidConnectorOptions = {}
): Connector {
  const metadata = plaidMetadata;
  const client = options.client ?? createDemoPlaidClient();
  ...
  const connector: Connector = {
    ...
    async connect(config) {
      const environment =
        (config.settings.environment as PlaidEnvironment | undefined) ?? "sandbox";
    ...
    async disconnect(instanceId) {
      ...
      if (config) plaidStore.clear(config.scope.organizationId);
      const state: ConnectorRuntimeState = {
        instanceId,
        connectorId: metadata.id,
        status: "disconnected",
    ...
    async authenticate(instanceId) {
      ...
      const environment =
        (config.settings.environment as PlaidEnvironment | undefined) ?? "sandbox";
      const publicToken = config.settings.publicToken as string | undefined;
      let accessToken =
        existing?.accessToken ??
        (config.settings.accessToken as string | undefined) ??
        "plaid-demo-access-token";

      if (publicToken) {
        const exchanged = await client.exchangePublicToken(publicToken);
        if (!exchanged.ok || !exchanged.accessToken) {
          return { ok: false, method: "oauth2", error: exchanged.error ?? "Public token exchange failed" };
        }
      ...
      const institutionId =
        ...
        existing?.secrets.institutionId ??
        undefined;

      const link = await client.createLinkToken({
        environment,
        clientUserId: config.scope.organizationId,
      });
      ...
      if (!remote.ok || !remote.session) {
        return { ok: false, method: "oauth2", error: remote.error ?? "Plaid auth failed" };
      }

      const institutions = await client.listInstitutions(accessToken);
      deps.persistence.saveConfiguration({
        ...config,
        settings: {
          ...config.settings,
          environment: remote.session.environment,
          institutionId: remote.session.institutionId,
          itemId: remote.session.itemId,
          linkToken: link.linkToken ?? remote.session.linkToken,
          linkExpiresAt: link.expiration ?? remote.session.expiresAt,
    ...
    async sync(request) {
      ...
      const jobId = createSyncJobId("plaid");
      ...
      try {
        ...
        const since =
          mode === "incremental"
            ? deps.cursors.get(request.instanceId, "plaid")?.cursor ?? null
            : null;

        const raw = await withRetry(async () => {
          retryCount += 1;
          const collected: PlaidRawEntity[] = [];
          for (const objectType of objectTypes) {
            let cursor: string | null = null;
            do {
              const page = await client.list(
                config.scope.organizationId,
                objectType as Parameters<PlaidClient["list"]>[1],
                since,
                cursor
              );
        ...
        plaidStore.replace(
          ...
          validation.accepted,
          {
            ...
            accountsImported: validation.accepted.filter(
              (r) => r.canonicalType === "finance.bank_account"
            ).length,
            transactionsImported: validation.accepted.filter(
              (r) => r.canonicalType === "finance.bank_transaction"
            ).length,
            failures: validation.rejected.length,
            retryCount,
            linkExpiresAt:
              (config.settings.linkExpiresAt as string | undefined) ??
              creds?.expiresAt ??
              creds?.secrets.expiresAt ??
              null,
            institutionHealth: remoteHealth.institutionHealth,
        ...
        if (newest) deps.cursors.set(request.instanceId, "plaid", newest);
        ...
        if (
          validation.accepted.some((r) => r.canonicalType === "finance.bank_transaction")
        ) {
          await deps.events.publish({
            ...
            scope: config.scope,
            payload: {
              count: validation.accepted.filter(
                (r) => r.canonicalType === "finance.bank_transaction"
              ).length,
            },
          });
        ...
        plaidStore.bumpFailure(config.scope.organizationId);
        deps.persistence.enqueueDeadLetter({
          instanceId: request.instanceId,
          connectorId: metadata.id,
          reason: message,
    ...
    async normalize(records: SyncRecord[], config: ConnectorConfiguration) {
      return normalizePlaidRecords(records, config);
    },

    async validate(records: NormalizedRecord[]): Promise<ValidationResult> {
      return validateNormalizedRecords(records, {
        requiredFields: {
          "finance.bank_account": ["id", "externalId", "organizationId"],
          "finance.bank_transaction": ["id", "externalId", "organizationId"],
          "finance.institution": ["id", "externalId", "organizationId"],
          "finance.bank_balance": ["id", "externalId", "organizationId"],
        },
      });
    },
    ...
    async healthCheck(instanceId): Promise<ConnectorHealthReport> {
      ...
      const snap = config ? plaidStore.get(config.scope.organizationId) : null;
