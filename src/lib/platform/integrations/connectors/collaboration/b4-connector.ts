/**
 * B4 Connector adapter for Slack / Teams / Zoom — populates collaboration store.
 */
...
import type { CollaborationProvider } from "@/lib/platform/integrations/connectors/collaboration/entities";
import {
...
export function createCollaborationB4Connector(
  metadata: ConnectorMetadata,
  provider: CollaborationProvider,
  deps: PlaceholderConnectorDeps
): Connector {
  const client = createDemoCollaborationClient(provider);
  ...
  const connector: Connector = {
    ...
    async disconnect(instanceId) {
      ...
      if (config) collaborationStore.clear(config.scope.organizationId, provider);
      const state: ConnectorRuntimeState = {
        instanceId,
        connectorId: metadata.id,
        status: "disconnected",
    ...
    async normalize(records: SyncRecord[], config: ConnectorConfiguration) {
      return normalizeCollaborationRecords(records, config, provider);
    },
    ...
    async validate(records: NormalizedRecord[]): Promise<ValidationResult> {
      return validateNormalizedRecords(records, {
        requiredFields: {
          ...
          "person.group": ["id", "externalId", "organizationId"],
        },
      });
    },
