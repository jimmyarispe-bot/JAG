...
} from "@/lib/platform/integrations/connectors/collaboration/services/demo-catalog";

export type CollaborationListPage = {
  records: CollaborationRawEntity[];
...
export interface CollaborationClient {
  ...
  list(
    ...
    since?: string | null,
    cursor?: string | null
  ): Promise<CollaborationListPage>;
  objectTypes(): CollaborationObjectType[];
}

export function createDemoCollaborationClient(
  ...
  return {
    ...
    async health() {
      return { ok: true, latencyMs: provider === "zoom" ? 40 : 22 };
    },
    ...
    objectTypes() {
      return objectTypesForProvider(provider);
    },
  };
}
