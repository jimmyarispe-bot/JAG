/**
 * Shared PlatformConnector factory for Canvas / PowerSchool / Google Classroom.
 */
...
import {
  ...
  toSyncRecords,
} from "@/lib/platform/integrations/connectors/education/normalization";
import {
...
export type EducationConnectorSpec = {
...
export function createEducationPlatformConnector(
  spec: EducationConnectorSpec,
  options: {
  ...
  const client = options.client ?? createDemoEducationClient(spec.provider);
  const orgFor =
  ...
  const metadata: ConnectorMetadata = {
    ...
    capabilities: [...spec.capabilities, "education-intelligence"],
  };
  ...
  const connector: PlatformConnector = {
    ...
    async disconnect(instanceId) {
      ...
      if (cfg) educationStore.clear(cfg.organizationId, spec.provider);
    ...
    async sync(request: SyncRequest): Promise<SyncResult> {
      ...
      try {
        ...
        const collected: EducationRawEntity[] = [];
        for (const objectType of objectTypes) {
          let cursor: string | null = null;
          do {
            const page = await client.list(
              cfg.organizationId,
              objectType as Parameters<EducationClient["list"]>[1],
              since,
              cursor
            );
        ...
        const normalized = normalizeEducationRecords(syncRecords, config, spec.provider);
        const canonical = normalized.map(
          (n) => n.data as unknown as EducationCanonicalEntity
        );

        educationStore.replace(
          cfg.organizationId,
          spec.provider,
          request.instanceId,
          normalized
        );
        buildEducationKnowledgeGraph(canonical);
        ingestConnectorGraph(cfg.organizationId);

        if (options.publisher) {
          for (const record of canonical) {
            if (record.objectType === "student" || record.objectType === "teacher") {
    ...
    async health(instanceId): Promise<HealthSnapshot> {
      const session = SESSIONS.get(`${spec.provider}:${instanceId}`);
      const cfg = instanceConfig.get(instanceId);
      const snap = cfg ? educationStore.get(cfg.organizationId, spec.provider) : null;
      ...
      return buildHealthSnapshot({
        ...
        errorCount: 0,
        message: `${spec.provider} education connector`,
      });
    },
  };
...
export async function reconnectEducationConnector(
