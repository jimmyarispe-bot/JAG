...
} from "@/lib/platform/integrations/connectors/education/services/demo-catalog";

export type EducationListPage = {
  records: EducationRawEntity[];
  nextCursor: string | null;
};

export interface EducationClient {
  readonly provider: EducationProvider;
  ...
  objectTypes(): EducationObjectType[];
}

export function createDemoEducationClient(provider: EducationProvider): EducationClient {
  const catalog = educationCatalogForProvider(provider);
  const PAGE_SIZE = 50;
  ...
  return {
    ...
    objectTypes() {
      return objectTypesForEducationProvider(provider);
    },
  };
}
