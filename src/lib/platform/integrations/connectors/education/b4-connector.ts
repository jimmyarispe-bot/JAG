/**
 * B4 Connector adapter for Education providers — populates educationStore.
 */
...
import { createDemoEducationClient } from "@/lib/platform/integrations/connectors/education/services/client";
import { educationStore } from "@/lib/platform/integrations/connectors/education/services/store";

export function createEducationB4Connector(
  metadata: ConnectorMetadata,
  provider: EducationProvider,
  deps: PlaceholderConnectorDeps
): Connector {
  const client = createDemoEducationClient(provider);
  ...
  const connector: Connector = {
    ...
    async disconnect(instanceId) {
      ...
      if (config) educationStore.clear(config.scope.organizationId, provider);
      const state: ConnectorRuntimeState = {
        instanceId,
        connectorId: metadata.id,
        status: "disconnected",
    ...
    async normalize(records: SyncRecord[], config: ConnectorConfiguration) {
      return normalizeEducationRecords(records, config, provider);
    },
    ...
    async validate(records: NormalizedRecord[]): Promise<ValidationResult> {
      return validateNormalizedRecords(records, {
        requiredFields: {
          ...
          "education.teacher": ["id", "externalId", "organizationId"],
          "education.course": ["id", "externalId", "organizationId"],
          "education.assignment": ["id", "externalId", "organizationId"],
          "education.grade": ["id", "externalId", "organizationId"],
        },
      });
    },
