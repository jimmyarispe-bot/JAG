/**
 * B4 Connector adapter for HR providers — populates hrStore.
 */
...
import { createDemoHrClient } from "@/lib/platform/integrations/connectors/hr/services/client";
import { hrStore } from "@/lib/platform/integrations/connectors/hr/services/store";

export function createHrB4Connector(
  metadata: ConnectorMetadata,
  provider: HrProvider,
  deps: PlaceholderConnectorDeps
): Connector {
  const client = createDemoHrClient(provider);
  ...
  const connector: Connector = {
    ...
    async disconnect(instanceId) {
      ...
      if (config) hrStore.clear(config.scope.organizationId, provider);
      const state: ConnectorRuntimeState = {
        instanceId,
        connectorId: metadata.id,
        status: "disconnected",
    ...
    async normalize(records: SyncRecord[], config: ConnectorConfiguration) {
      return normalizeHrRecords(records, config, provider);
    },
    ...
    async validate(records: NormalizedRecord[]): Promise<ValidationResult> {
      return validateNormalizedRecords(records, {
        requiredFields: {
          ...
          "hr.payroll": ["id", "externalId", "organizationId"],
          "hr.department": ["id", "externalId", "organizationId"],
          "hr.manager": ["id", "externalId", "organizationId"],
        },
      });
    },
