/**
 * Shared PlatformConnector factory for ADP / Gusto / Paylocity / BambooHR.
 */
...
import {
  normalizeHrRecords,
  toSyncRecords,
} from "@/lib/platform/integrations/connectors/hr/normalization";
import {
  createDemoHrClient,
  type HrClient,
...
export function createHrPlatformConnector(
  spec: HrConnectorSpec,
  options: {
    client?: HrClient;
  ...
  const client = options.client ?? createDemoHrClient(spec.provider);
  const orgFor =
  ...
  const metadata: ConnectorMetadata = {
    ...
    capabilities: [...spec.capabilities, "hr-intelligence"],
  };
  ...
  const connector: PlatformConnector = {
    ...
    async disconnect(instanceId) {
      ...
      if (cfg) hrStore.clear(cfg.organizationId, spec.provider);
    ...
    async sync(request: SyncRequest): Promise<SyncResult> {
      ...
      try {
        ...
        const collected: HrRawEntity[] = [];
        for (const objectType of objectTypes) {
          let cursor: string | null = null;
          do {
            const page = await client.list(
              cfg.organizationId,
              objectType as Parameters<HrClient["list"]>[1],
              since,
              cursor
            );
        ...
        const normalized = normalizeHrRecords(syncRecords, config, spec.provider);
        const canonical = normalized.map(
          (n) => n.data as unknown as HrCanonicalEntity
        );

        hrStore.replace(cfg.organizationId, spec.provider, request.instanceId, normalized);
        buildHrKnowledgeGraph(canonical);
        ingestConnectorGraph(cfg.organizationId);

        if (options.publisher) {
          for (const record of canonical) {
            if (record.objectType === "employee" || record.objectType === "manager") {
    ...
    async health(instanceId): Promise<HealthSnapshot> {
      const session = SESSIONS.get(`${spec.provider}:${instanceId}`);
      const cfg = instanceConfig.get(instanceId);
      const snap = cfg ? hrStore.get(cfg.organizationId, spec.provider) : null;
      ...
      return buildHealthSnapshot({
        ...
        errorCount: 0,
        message: `${spec.provider} HR connector`,
      });
    },
  };
...
export async function reconnectHrConnector(
