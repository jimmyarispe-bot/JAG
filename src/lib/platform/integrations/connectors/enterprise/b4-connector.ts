/**
 * B4 Connector adapter for enterprise providers — populates enterpriseStore.
 */
...
export function createEnterpriseB4Connector(
  metadata: ConnectorMetadata,
  provider: EnterpriseProvider,
  deps: PlaceholderConnectorDeps
): Connector {
  const client = createDemoEnterpriseClient(provider);
  ...
  const connector: Connector = {
    ...
    async disconnect(instanceId) {
      ...
      if (config) enterpriseStore.clear(config.scope.organizationId, provider);
      const state: ConnectorRuntimeState = {
        instanceId,
        connectorId: metadata.id,
        status: "disconnected",
    ...
    async normalize(records: SyncRecord[], config: ConnectorConfiguration) {
      return normalizeEnterpriseRecords(records, config, provider);
    },
    ...
    async validate(records: NormalizedRecord[]): Promise<ValidationResult> {
      return validateNormalizedRecords(records, {
        requiredFields: {
          ...
          "gov.program": ["id", "externalId", "organizationId"],
        },
      });
    },
