/**
 * B4 Connector adapter for finance providers — populates financeStore.
 * Used to promote Stripe from placeholder; QB/Square/Plaid keep existing B4 connectors.
 */
...
import type { FinanceProvider } from "@/lib/platform/integrations/connectors/finance/entities";
import {
...
export function createFinanceB4Connector(
  metadata: ConnectorMetadata,
  provider: FinanceProvider,
  deps: PlaceholderConnectorDeps
): Connector {
  const client = createDemoFinanceClient(provider);
  ...
  const connector: Connector = {
    ...
    async disconnect(instanceId) {
      ...
      if (config) financeStore.clear(config.scope.organizationId, provider);
      const state: ConnectorRuntimeState = {
        instanceId,
        connectorId: metadata.id,
        status: "disconnected",
    ...
    async authenticate(instanceId) {
      ...
      return authenticatePlaceholder(deps.credentials, {
        instanceId,
        method: provider === "stripe" ? "api_key" : "oauth2",
        secrets: {
    ...
    async normalize(records: SyncRecord[], config: ConnectorConfiguration) {
      return normalizeFinanceRecords(records, config, provider);
    },
    ...
    async validate(records: NormalizedRecord[]): Promise<ValidationResult> {
      return validateNormalizedRecords(records, {
        requiredFields: {
          ...
          "finance.subscription": ["id", "externalId", "organizationId"],
