...
export async function getConvertibleLeads(): Promise<ConvertibleList> {
  ...
  const [studentsRes, schoolsRes] = await Promise.all([
    supabase
      .from("students")
      .select("id, first_name, last_name, school_id, admissions_lead_id")
      .in("school_id", schoolIds.length > 0 ? schoolIds : ["00000000-0000-0000-0000-000000000000"]),
  ...
  for (const s of students) {
    ...
    const key = `${s.school_id}|${nameKey(s.first_name as string | null, s.last_name as string | null)}`;
  ...
  const rows: ConvertibleLead[] = leads.map((lead) => {
    ...
    const matches = byName.get(
      `${lead.school_id}|${nameKey(lead.first_name as string | null, lead.last_name as string | null)}`
    ) ?? [];
    ...
    if (linkedStudentId) {
      ...
      verdict = "ambiguous";
      verdictNote = `${matches.length} students in this campus share this name. A person has to say which one. No button until then.`;
    } else if (matches.length === 1) {
      verdict = "link";
      studentId = matches[0];
      verdictNote = "Already on the roster. This will LINK the lead to that student - it will not create a second one.";
    } else {
      verdict = "create";
