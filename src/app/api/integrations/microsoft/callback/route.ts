...
function settingsRedirect(query: Record<string, string>): NextResponse {
  ...
  const url = new URL("/settings/integrations/microsoft", base);
 ...
 * Microsoft Entra OAuth return — verifies signed state, session, org membership, then persists.
 */
export async function GET(request: NextRequest) {
  ...
  const tokens = await exchangeMicrosoftAuthorizationCode(code);
  if ("error" in tokens) {
    return settingsRedirect({ error: tokens.error, connected: "0" });
  }

  const saved = await upsertMicrosoft365Connection(supabase, {
  ...
  await logSecurityEvent(supabase, {
    ...
    summary: "Microsoft 365 OAuth connection established",
