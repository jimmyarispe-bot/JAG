-- 335_interest_form_v10_goal_pledge_2026_09_12.sql
--
-- Version 10: the GA GOAL taxpayer pledge, signed.
--
-- Every Georgia family signs it, whichever scholarships they go on to tick — it
-- sits immediately above that question and is not conditional on the answer.
-- The pledge is about how the fund is filled, not about what this family draws
-- from it, and asking only the families who take GOAL money to support GOAL
-- money would be asking the wrong half of the room.
--
-- A NEW FIELD TYPE, BECAUSE `consent` IS A CHECKBOX. A tick is not a signature.
-- `signature` renders the statement as body text — not as a small grey form
-- label, which is where long text goes to be ignored — with a name field
-- beneath it and a line saying plainly that typing a full name is the
-- signature.
--
-- NO DATE FIELD. The submission already records `submitted_at`. A second date
-- the parent can type is a date that can disagree with the record, and of the
-- two the one nobody typed is the one worth trusting.
--
-- WHAT THIS IS AND IS NOT. It is an acknowledgement with a name attached,
-- stored in `admissions_interest_answers` against a specific form version, so
-- what was signed can always be reconstructed. It is not a witnessed signature
-- and it is not a contract. The enrollment contract is a separate document with
-- its own execution, and nothing here changes that.
--
-- REQUIRES THE CODE THAT RENDERS `signature`. Ship first: an unknown field type
-- falls through to a plain text input, so the statement would render as a form
-- label above an unexplained box — present, technically answerable, and read by
-- nobody.
--
-- Version 9 is archived, not deleted. Safe to re-run.
  ...
  definition_text := $json$
{
  ...
  "questions": [
    ...
    },
    {
      ...
      "type": "signature",
      "label": "I understand GA GOAL student scholarship funds are derived from GA tax payers' contributions. Instead of giving GA tax liabilities to the state government, taxpayers can designate these funds (Single filer or head of household: Up to $2,500; Married filing separately: Up to $2,500; Married couple filing jointly: Up to $5,000) to a GA GOAL school, such as The Academy GA. I understand this and as a GA resident who pays state taxes, I pledge to help support this GA GOAL student scholarship fund so even more students can get the financial assistance to attend The Academy GA.",
      "required": true,
      "order": 41,
      "systemBinding": null
    }
...
-- Expect version 10, 11 sections, 54 questions, 0 unresolved campus tokens.
select
