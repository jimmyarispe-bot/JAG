-- 353_interest_form_v19_pledge_label_2026_09_14.sql
--
-- Version 19: the GA GOAL signature box said "Signature" twice.
--
-- v18 moved the pledge wording into the section description and left the
-- question's own label as "Signature". The signature field renders the label as
-- a paragraph AND a hard-coded "Signature *" above the input, so the box opened
-- with a quiet unbolded "Signature" sitting above the real, bolded one.
--
-- The label is now empty. Two guards went in alongside it, because an empty
-- label was not previously a safe thing to have:
--
--   - The renderer omits the paragraph entirely when there is no label, rather
--     than printing an empty one and holding the space.
--   - Required-field validation fell back to `${question.label} is required.`,
--     which with no label reads " is required." and names nothing. It now says
--     "Signature is required." for a signature, "This field is required."
--     otherwise.
--
-- Nothing else changes: same key, same requiredness, same gate, same section.
-- Every answer already recorded still means what it meant.
--
-- Version 18 is archived, not deleted. Safe to re-run.
...
-- Expect version 19, 12 sections, 59 questions, 0 unresolved campus tokens.
select
