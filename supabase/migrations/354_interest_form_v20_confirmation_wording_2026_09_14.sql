-- 354_interest_form_v20_confirmation_wording_2026_09_14.sql
--
-- Version 20: the confirmation a family signs covers the application, not just
-- the GA GOAL eligibility ticks, and says what is actually at stake.
--
-- BEFORE:
--   "I confirm that the eligibility criteria I have ticked above are true for my
--    student, and that the documents I have uploaded are genuine. I understand
--    that GA GOAL verifies eligibility and that a scholarship may be withdrawn if
--    this information proves inaccurate."
--
-- AFTER: the whole application rather than the checkboxes above it; the school
-- verifies rather than GA GOAL; and removal from the school sits alongside
-- withdrawal of the scholarship, which is the consequence that was missing.
--
-- Wording supplied by Jimmy, reproduced as given.
--
-- Nothing else changes. Same key, same requiredness, same gate — it stays inside
-- the eligibility section, which is still shown only to families who tick GA
-- GOAL Scholarship. Every answer already recorded still means what it meant.
--
-- Version 19 is archived, not deleted. Safe to re-run.
...
-- Expect version 20, 12 sections, 59 questions, 0 unresolved campus tokens.
select
