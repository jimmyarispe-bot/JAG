-- 352_interest_form_v18_ga_goal_fund_2026_09_14.sql
--
-- Version 18: the GA GOAL ask gets its own heading and its own words.
--
-- WHAT CHANGES, all of it copy and layout on the Georgia campus section:
--
--   1. "The Academy GA" and "A few questions specific to our Georgia campus."
--      are gone. A family that has already chosen the campus does not need to be
--      told which campus they are on, two screens later.
--
--   2. The GA GOAL pledge gains a heading of its own,
--      "GA GOAL - Student Scholarship Fund", and Jimmy's rewritten text.
--
--      The old wording opened "I understand GA GOAL student scholarship funds
--      are derived from GA tax payers' contributions" — a paragraph of tax
--      mechanics ending in a pledge. The new one says the same thing as an ask:
--      this means YOU, here is what you can give, please help each year.
--
--   3. The pledge's own label is now just "Signature", because the section
--      description carries the text the label used to.
--
-- WHO SEES IT, unchanged. The pledge has never had a visibleWhen of its own and
-- sat inside a section gated on The Academy GA, so it already appeared for every
-- GA family whether or not they ticked GA GOAL Scholarship. The new section
-- carries the same gate. Confirmed with Jimmy before writing this: the pledge is
-- for every GA family, and only GA families.
--
-- The eligibility section — waiver routes, proof of income, proof of eligibility
-- — is deliberately NOT touched. It stays gated on ticking GA GOAL, because both
-- of its uploads are required and showing it to everyone would stop any family
-- without a tax return to hand from submitting at all.
--
-- WHY ga_detail BECOMES THREE SECTIONS. Sections render in `order` and a heading
-- belongs to a section, so the GA GOAL block needs to be one. Splitting keeps the
-- pledge exactly where it has always been on the page — after the zoned-school
-- question, before the scholarships checkboxes — rather than moving it to the
-- bottom of the campus block, which is where a single appended section would have
-- put it. The two halves of the old section carry empty titles; the renderer now
-- omits the heading entirely when a section has neither title nor description.
--
-- No question keys change, so every answer already recorded still means what it
-- meant. Version 17 is archived, not deleted. Safe to re-run.
  ...
  definition_text := $json$
{
  ...
  "questions": [
    ...
    },
    {
      ...
      "type": "signature",
      "label": "I confirm that the eligibility criteria I have ticked above are true for my student, and that the documents I have uploaded are genuine. I understand that GA GOAL verifies eligibility and that a scholarship may be withdrawn if this information proves inaccurate.",
      "required": true,
      "order": 53,
      "systemBinding": null,
      "visibleWhen": {
...
-- Expect version 18, 12 sections, 59 questions, 0 unresolved campus tokens.
select
