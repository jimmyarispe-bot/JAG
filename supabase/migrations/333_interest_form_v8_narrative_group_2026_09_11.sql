-- 333_interest_form_v8_narrative_group_2026_09_11.sql
--
-- Version 8: the four questions about a child, asked together.
--
-- WHY. They were scattered, and three of them existed once per campus asking
-- the identical thing: `fl_peer_interaction` and `ga_peer_interaction` are the
-- same question, as are `fl_anything_else`, `ga_anything_else` and
-- `virtual_anything_else`. Meanwhile greatness and challenges sat up in
-- "Program & School", between the campus picker and a scholarship question.
--
-- A parent writing about their child was doing it in two places with a form
-- section in between. Now there is one section, "About your child", holding all
-- four.
--
-- FIVE QUESTIONS BECOME TWO. `peer_interaction` and `anything_else` replace the
-- per-campus copies. Identical text answered under three different keys is
-- three different answers as far as any report is concerned, which is how a
-- question nobody can aggregate gets asked three times.
--
-- WHAT CHANGES FOR HS, WHICH IS ALMOST NOTHING. The section is visible to
-- everyone, so HS still asks for greatness and challenges under the same keys
-- as before. The two questions HS does not ask carry their own condition —
-- present, and not the high school. HS's own parent questions and its
-- student-authored section are untouched.
--
-- The one visible difference for an HS family is the heading those two
-- questions now sit under. Their wording, their keys, and their answers are
-- unchanged.
--
-- WHY THE CONDITION TESTS `exists` FIRST. `neq` against the HS id is true for
-- an empty answer, so a family who has not yet chosen a campus would see the
-- two questions appear and then vanish when they picked the high school.
-- Requiring the answer to exist first means the section fills in as they
-- choose, rather than flickering.
--
-- ORDERING. Greatness, then challenges, then how they get on with other
-- children, then anything else. "Anything else" belongs last; it is the
-- question that means "and now whatever we did not think to ask".
--
-- RENAMED KEYS ORPHAN ANSWERS. The five retired keys were published in v4 this
-- evening. Any answer already filed under them stays in
-- `admissions_interest_answers` against its own form version and is still
-- readable there — it simply will not aggregate with answers to the new keys.
-- At this age that is a handful of test rows; in a month it would not be.
--
-- Version 7 is archived, not deleted. Safe to re-run.
...
-- Expect version 8, 11 sections, 53 questions, 0 unresolved campus tokens.
select
