...
export const emptyEthicalScope = (): GraphScope => ({ organizationId: "org-unknown", schoolId: "school-unknown" });
...
const lightScore = (value: number | undefined, fallback: number) => clamp(typeof value === "number" ? value : fallback);

export function deriveEthicalBaseline(request: EthicalRequest): EthicalBaseline {
  const base = defaultEthicalBaseline();
  const health = lightScore(request.dnaResult?.healthScore?.value ?? request.oiosResult?.baseline?.organizationHealthScore, base.organizationHealthScore);
...
console.log("models.ts written", fs.statSync(path.join(DEST, "models.ts")).size);