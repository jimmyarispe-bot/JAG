...
describe("JAG platform user access", () => {
  ...
  it("denies unauthorized roles from /jag/users", () => {
    for (const role of ["CEO", "JAG_ORG_STAFF", "JAG_ORG_ADMIN", "TEACHER"]) {
  ...
  it("leaves AcademyOS user-management catalog unchanged", () => {
    ...
    expect(values).toContain("JAG_ORG_STAFF");
  });

  it("does not use JAG_ORG_STAFF as the JAG platform grant", () => {
    expect(JAG_PLATFORM_GRANT_ROLE).not.toBe("JAG_ORG_STAFF");
    expect(
      hasJagPlatformAccess(["JAG_ORG_STAFF"])
    ).toBe(false);
  });
